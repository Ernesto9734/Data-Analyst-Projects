...
def exploracion_inicial_datos_categoricos(datos: pd.DataFrame, columnas: list[str]) -> None:
    """
    Realiza un perfilamiento estadístico inicial de variables categóricas,
    evaluando cardinalidad, unicidad, valores ausentes y distribución básica.

    Args:
        datos (pd.DataFrame): DataFrame con los datos.
        columnas (list[str]): Variables categóricas a analizar.

    Returns:
        None: Imprime el reporte exploratorio.
    """
    ...
    print(f'Total de Variables No Numéricas Analizadas: {total_variables}\n')
    ...
    for col in columnas:
        ...
        indice_unicidad = valores_unicos / total_registros


        print(f'Cantidad de Valores Únicos (Cardinalidad): {valores_unicos}')
        print(f'Índice de Unicidad: {indice_unicidad:.4f}')
        print(f'Valores Faltantes (NaN): {total_nulos} ({porcentaje_nulos:.2f}%)')


        # Categoría dominante
        if valores_unicos > 0:
            frecuencia = datos[col].value_counts(dropna=True)

            categoria_dominante = frecuencia.index[0]
            frecuencia_dominante = frecuencia.iloc[0]

            porcentaje_dominante = (
                frecuencia_dominante / total_registros
            ) * 100

            print(
                f'Categoría Dominante: {categoria_dominante}'
            )
            print(
                f'Frecuencia Dominante: {frecuencia_dominante} '
                f'({porcentaje_dominante:.2f}%)'
            )


        muestra_valores = datos[col].dropna().unique()[:5]

        print(f'Muestra de Valores: {muestra_valores}')
        print()